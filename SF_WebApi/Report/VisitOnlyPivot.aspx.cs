...
{
    public partial class VisitOnlyPivot : System.Web.UI.Page
    {
        ...
        {
            //startDate.Value = string.Format(System.Globalization.CultureInfo.GetCultureInfo("en-ENG"), "yyyy-MM-dd", startDate.Value);
            //endDate.Value = string.Format(System.Globalization.CultureInfo.GetCultureInfo("en-ENG"), "yyyy-MM-dd", endDate.Value);
            
            //ASPxPivotGrid1.ReloadData();
            //Response.Redirect("~/Report/VisitOnlyPivot.aspx?p=" + txtPosition.Text + "&r=" + txtNik.Text + "&s=" + startDate.Text + "&e=" + endDate.Text);
        }
        ...
        {
            //if(ASPxPivotGridExporter1.PivotGrid != null)
            //ASPxPivotGridExporter1.OptionsPrint.PrintFilterHeaders = DevExpress.Utils.DefaultBoolean.False;
            //ASPxPivotGridExporter1.ExportXlsxToResponse("VisitOnlyPivot", new XlsxExportOptionsEx
            //{
            ...
            var addressPath = headerPath + "/" + txtNik.Text + "/VisitOnlyPivot"; // ~/Asset/Files/Downloads/Pdf/VisitRealization/12.36
            var resultFileName = "visitonlypivot.xlsx";
        ...
        protected void BtnExportDataRow_Click(object sender, EventArgs e)
        {
            ...
            var headerPath = (string)settingsReader.GetValue("ReportPath", typeof(String)); //~/Asset/Files/Downloads/Pdf/
            var addressPath = headerPath + "/" + txtNik.Text + "/VisitOnlyPivot"; // ~/Asset/Files/Downloads/Pdf/VisitRealization/12.36
            var resultFileName = "visitonlypivot_rowdata.xlsx";
            var resultFilePath = addressPath + "/" + resultFileName;
            if (!Directory.Exists(HttpContext.Current.Server.MapPath(addressPath)))
            {
                Directory.CreateDirectory(HttpContext.Current.Server.MapPath(addressPath));
            ...
            var hostPath = hostLink.Replace("bas_api_mobile", String.Empty) + resultFilePath; //http://tanabe-id.intra.sharedom.net/bas_api_mobile/ReportPath/Files/Report/12.36/VisitOnlyPivot/visitonlypivot_rowdata.xlsx
            Response.Redirect(hostPath);
            //Response.Write("<script>window.open('" + (string)settingsReader.GetValue("host", typeof(String)) + resultFilePath + "','_blank');</script>");
        }

        protected void BtnExportPdf_Click(object sender, EventArgs e)
        {
            //ASPxPivotGridExporter1.ExportPdfToResponse("VisitOnlyPivot", new PdfExportOptions()
            //{
            ...
            var headerPath = (string)settingsReader.GetValue("ReportPath", typeof(String)); //~/Asset/Files/Downloads/Pdf/
            var addressPath = headerPath + "/" + txtNik.Text + "/VisitOnlyPivot"; // ~/Asset/Files/Downloads/Pdf/VisitRealization/12.36
            var resultFileName = "visitonlypivot.pdf";
            var resultFilePath = addressPath + "/" + resultFileName;
            if (!Directory.Exists(HttpContext.Current.Server.MapPath(addressPath)))
            {
                Directory.CreateDirectory(HttpContext.Current.Server.MapPath(addressPath));
