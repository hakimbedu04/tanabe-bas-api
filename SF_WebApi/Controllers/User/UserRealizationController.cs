...
{
    public class UserRealizationController : BaseController
    {
        private readonly IUserRealizationBLL _bll;
        private readonly ILoginBLL _loginbll;
        private readonly IVTLogger _vtLogger;

        public UserRealizationController(IUserRealizationBLL bll, ILoginBLL loginbll, IVTLogger vtLogger)
        {
            _bll = bll;
            _loginbll = loginbll;
            _vtLogger = vtLogger;
        ...
        [HttpPost]
        public HttpResponseMessage ViewUserRealization(UserInputs inputs)
        {
            ...
            try
            {
                ...
                List<v_sales_product_DTO> dbResult = _bll.GetUserRealizationDatas(inputs);
        ...
        [HttpPost]
        public HttpResponseMessage DetailUserRealization(UserInputs inputs)
        {
            var objResponseModel = new ResponseModel();
            try
            {
                ...
                inputs.RepId = Decrypt(inputs.Auth, true);
                List<t_sales_product_actual_DTO> dbResult = _bll.GetSalesProductActualDtos(inputs);
                var viewMapper = Mapper.Map<List<t_sales_product_actual_ViewModel>>(dbResult);
                objResponseModel.Status = true;
                objResponseModel.Message = EnumHelper.GetDescription(Enums.ResponseType.Success);
                if (viewMapper.Count != 0)
                {
        ...
        [HttpPost]
        public HttpResponseMessage AddProduct(UserInputs inputs)
        {
            var objResponseModel = new ResponseModel();
            try
            {
                ...
                inputs.RepId = Decrypt(inputs.Auth, true);
                _bll.InsertSalesProductActual(inputs);
                List<t_sales_product_actual_DTO> dbResult = _bll.GetSalesProductActualDtos(inputs);
                var viewMapper = Mapper.Map<List<t_sales_product_actual_ViewModel>>(dbResult);
                objResponseModel.Status = true;
                objResponseModel.Message = EnumHelper.GetDescription(Enums.ResponseType.Success);
        ...
        [HttpPost]
        public HttpResponseMessage UpdateProduct(UserInputs inputs)
        {
            var objResponseModel = new ResponseModel();
            try
            {
                ...
                inputs.RepId = Decrypt(inputs.Auth, true);
                _bll.UpdateSalesProductActual(inputs);
                List<t_sales_product_actual_DTO> dbResult = _bll.GetSalesProductActualDtos(inputs);
                var viewMapper = Mapper.Map<List<t_sales_product_actual_ViewModel>>(dbResult);
                objResponseModel.Status = true;
                objResponseModel.Message = EnumHelper.GetDescription(Enums.ResponseType.Success);
        ...
        [HttpPost]
        public HttpResponseMessage DeleteProduct(UserInputs inputs)
        {
            var objResponseModel = new ResponseModel();
            try
            {
                ...
                inputs.RepId = Decrypt(inputs.Auth, true);
                _bll.DeleteSalesProductActual(inputs);
                List<t_sales_product_actual_DTO> dbResult = _bll.GetSalesProductActualDtos(inputs);
                var viewMapper = Mapper.Map<List<t_sales_product_actual_ViewModel>>(dbResult);
                objResponseModel.Status = true;
                objResponseModel.Message = EnumHelper.GetDescription(Enums.ResponseType.Success);
        ...
        [HttpPost]
        public HttpResponseMessage ExportToPdf(UserExportCustomModelInputs inputs)
        {
            var objResponseModel = new ResponseModel();
            bool validateAccess = ValidateAuth(inputs.Auth);
            if (!validateAccess)
            {
                ...
                Request.CreateResponse(HttpStatusCode.InternalServerError, objResponseModel);
            }
        ...
        [HttpPost]
        public HttpResponseMessage ExportToExcel(UserExportCustomModelInputs inputs)
        {
            var objResponseModel = new ResponseModel();
            bool validateAccess = ValidateAuth(inputs.Auth);
            if (!validateAccess)
        ...
        [HttpPost]
        public string GenerateExcel(UserExportCustomModelInputs inputs)
        {
            inputs.RepId = Decrypt(inputs.Auth, true);
            ...
            string addressPath = headerPath + "UserRealization" + "/" + model.rep_id;
                // ~/Asset/Files/Downloads/Pdf/VisitRealization/12.36
            string resultFileName = "User_Realization_" + GetStringPattern().Replace(model.rep_name, "_") + "_" +
            ...
            string templateFile = Enums.ExcelTemplate.User_Realization + ".xlsx";
            string templateFileName = HttpContext.Current.Server.MapPath(Constants.UserRealization + templateFile);
            ...
            using (var slDoc = new SLDocument(strFileName))
            {
                ...
                foreach (v_sales_product_DTO item in dbResult)
                {
                    #region sub header
                    ...
                    slDoc.SetCellValue(iRow, 2, item.dr_code.HasValue ? Convert.ToString(item.dr_code.Value) : "");
                    slDoc.SetCellValue(iRow, 3, item.sales_realization.HasValue ? item.sales_realization.Value : 0);
                    slDoc.SetCellValue(iRow, 4, item.dr_spec);
                    slDoc.SetCellValue(iRow, 5, item.dr_quadrant);
                    slDoc.SetCellValue(iRow, 6, item.dr_monitoring);
                    slDoc.SetCellValue(iRow, 7, item.prd_name);
        ...
        public string GeneratePdf(UserExportCustomModelInputs inputs)
        {
            ...
            title.Add(new Paragraph("USER REALIZATION ", TitleFont));
            ...
            dt.Columns.Add("User Real", typeof (string));
            ...
            foreach (v_sales_product_DTO item in dbResult)
            {
                ...
                dr["DR. CODE"] = item.dr_code.HasValue ? Convert.ToString(item.dr_code.Value) : "";
                dr["User Real"] = item.sales_realization;
            ...
            float[] widths = {3f, 3f, 3f, 3f, 4f, 10f, 10f, 3f, 4f, 4f, 4.5f, 3.5f, 4f};
