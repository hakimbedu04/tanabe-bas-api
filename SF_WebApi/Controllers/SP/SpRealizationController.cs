...
{
    public class SpRealizationController : BaseController
    {
        private readonly ILoginBLL _loginbll;
        private readonly ISPRealizationBLL _mainBLL;
        private readonly ISPPlanBLL _planBll;
        private readonly IVisitBLL _visitBll;
        private readonly IVTLogger _vtLogger;

        public SpRealizationController(ILoginBLL loginbll, ISPRealizationBLL mainBLL, ISPPlanBLL planBll,
            IVisitBLL visitBll, IVTLogger vtLogger)
        {
            _loginbll = loginbll;
            _mainBLL = mainBLL;
            _planBll = planBll;
        ...
        public HttpResponseMessage DataViewPartial(SPInputs inputs)
        {
            ...
            try
            {
                ...
                List<SP_SELECT_SP_REALIZATION_DTO> dbResult = _mainBLL.GetSPRealization(inputs);
                var viewMapper = Mapper.Map<List<SP_SELECT_SP_REALIZATION_ViewModel>>(dbResult);
        ...
        [HttpPost]
        public HttpResponseMessage DoctorDetailPartialView(SPInputs inputs)
        {
            var objResponseModel = new ResponseModel();
            try
            {
                ...
                List<SP_SELECT_DOCTOR_LIST_SP_DTO> dbResult = _mainBLL.GetDoctorList(inputs);
        ...
        [HttpPost]
        public HttpResponseMessage SponsorGeneralPartialView(SPInputs inputs)
        {
            var objResponseModel = new ResponseModel();
            try
            {
                ...
                List<m_sponsor_DTO> dbResult = _planBll.GetSponsor(inputs);
        ...
        [HttpPost]
        public HttpResponseMessage SaveAdditional(SPInputs inputs)
        {
            var objResponseModel = new ResponseModel();
            try
            {
                ...
                bool addSpeaker = _mainBLL.AddSpeakerAdditional(inputs);
                if (addSpeaker)
                {
                    objResponseModel.Status = true;
                    objResponseModel.Message = EnumHelper.GetDescription(Enums.ResponseType.SuccessAdditionalRealization);
                }
                else
                {
                    objResponseModel.Status = false;
                    objResponseModel.Message = EnumHelper.GetDescription(Enums.ResponseType.SaveError);
                }
                List<v_doctor_sponsor_DTO> dbResult = _planBll.GetSPParticipant(inputs);
                var viewMapper = Mapper.Map<List<v_doctor_sponsor_DTO>>(dbResult);
                objResponseModel.TotalRecords = viewMapper.Count;
                objResponseModel.Result = viewMapper;
                return Request.CreateResponse(HttpStatusCode.OK, objResponseModel);
            }
        ...
        [HttpPost]
        public HttpResponseMessage BatchUpdateParticipantPartial(SPInputs inputs)
        {
            var objResponseModel = new ResponseModel();
            bool validateAccess = ValidateAuth(inputs.Auth);
            if (!validateAccess)
            ...
            foreach (Collection item in inputs.BatchValues)
            {
                if (item.BudgetRealValue != 0)
                {
                    try
                    {
                        if (_mainBLL.UpdateRealization(item))
                        {
                            objResponseModel.Status = true;
                            objResponseModel.Message = EnumHelper.GetDescription(Enums.ResponseType.Success);
                        }
                    ...
                    catch (Exception ex)
                    {
                        ...
                        objResponseModel.Message =
                            EnumHelper.GetDescription(msg.Contains("real_value_null")
                                ? Enums.ResponseType.RealValueNull
        ...
        [HttpPost]
        public HttpResponseMessage SubmitRealization(SPInputs inputs)
        {
            var objResponseModel = new ResponseModel();
            try
            {
                ...
                inputs.RepId = Decrypt(inputs.Auth, true);
                bool saveRealization = _mainBLL.SubmitRealization(inputs);
                if (saveRealization)
                {
                    SendRequestVerification(inputs);
                    objResponseModel.Status = true;
                    objResponseModel.Message = EnumHelper.GetDescription(Enums.ResponseType.Success);
                }
                else
        ...
        public void SendRequestVerification(SPInputs inputs)
        {
            bas_v_rep_fullDTO userDetail = _loginbll.CheckMvaUserInfo(inputs.RepId);
            List<v_rep_admin_DTO> modulReq = _visitBll.GetDataAdmin(inputs.RepId);
            var reader =
                new StreamReader(
            ...
            foreach (v_rep_admin_DTO receiver in modulReq)
            {
                email_body = email_body.Replace("$$RECEIVER$$", receiver.admin_name);
                email_body = email_body.Replace("$$spr_no$$", inputs.SprId);
                ...
                if (!String.IsNullOrEmpty(receiver.admin_email))
                {
                    mailMessage.From = new MailAddress(key);
                    mailMessage.To.Add(new MailAddress(receiver.admin_email)); 
                    ////mailMessage.To.Add(new MailAddress("abdurahman.hakim@vodjo.com"));
                    ////mailMessage.To.Add(new MailAddress("edi.suherman@vodjo.com"));

                    mailMessage.Subject = "Sales Promotion Verification Request for " + userDetail.rep_name + " - " +
        ...
        public string GeneratePdf(SPInputs inputs)
        {
            ...
            string addressPath = headerPath + "SpRealization" + "/" + model.rep_id;
                // ~/Asset/Files/Downloads/Pdf/VisitRealization/12.36
            string resultFileName = "sp_realization" + GetStringPattern().Replace(model.rep_name, "_") + "_" +
            ...
            title.Add(new Paragraph("SALES PROMOTION REALIZATION ", TitleFont));
            ...
            string tempField = "";
            foreach (SP_SELECT_SP_REALIZATION_ViewModel item in viewMapper)
            {
        ...
        public string GenerateExcel(SPInputs inputs)
        {
            ...
            string addressPath = headerPath + "SpRealization" + "/" + model.rep_id; // folder asset sampai rep_id
            string resultFileName = "Sp_Realization_" + GetStringPattern().Replace(model.rep_name, "_") + "_" +
            ...
            string templateFile = Enums.ExcelTemplate.SP_Realization + ".xlsx";
            string templateFileName = HttpContext.Current.Server.MapPath(Constants.SPRealization + templateFile);
        ...
        public string GenerateExcels(SPInputs inputs)
        {
            inputs.RepId = Decrypt(inputs.Auth, true);
            bas_v_rep_fullDTO model = _loginbll.CheckMvaUserInfo(inputs.RepId);
            List<SP_SELECT_SP_REALIZATION_DTO> dbResult = _mainBLL.GetSPRealization(inputs);
        ...
        [HttpPost]
        public HttpResponseMessage UploadAttachment() // blm selesai semua
        {
            var objResponseModel = new ResponseModel();
            string auth = Convert.ToString(HttpContext.Current.Request.Params["Auth"]);
            string sprid = HttpContext.Current.Request.Params["SprId"];
            ...
            try
            {
                ...
                if (httpRequest.Files.Count > 0)
                {
                    foreach (string file in httpRequest.Files)
                    {
                        ...
                        var settingsReader = new AppSettingsReader();
                        //string headerPath =
                        //    (string) settingsReader.GetValue("UploadAttachmentSpPath", typeof (String)) + rep_reg + "/" +
                        //    repid + "/";
                        //bas/Files/SP-Attachment/SP_Files/12.04/SPR1906611-SPR1906611_20190822_131933_125.jpg --> UploadAttachmentSpPath
                        //bas/WebVirtualDir/SP-Attachment/SP_Files/R-2/07.18/V19174505-V19174505_263004_20190913_095321_378.jpg --> UploadAttachmentPath
                        string headerPath = (string)settingsReader.GetValue("UploadAttachmentPath", typeof(String)) + "SP-Attachment/SP_Files/" + rep_reg + "/" + repid + "/";
                        ...
                        {
                            string fileName = sprid + "-" + httpRequest.Files[file].FileName;
                            ...
                            _visitBll.InsertSPRealizationAttachment(sprid, fileName, headerPath.Replace("WebVirtualDir", "Files") + fileName);
                        }
        ...
        [HttpPost]
        public HttpResponseMessage gridAttachmentView(SPInputs inputs)
        {
            var objResponseModel = new ResponseModel();
            try
            {
                ...
                inputs.RepId = Decrypt(inputs.Auth, true);
                List<t_sp_attachment_DTO> dbResult = _mainBLL.GetSPAttachment(inputs);
                var viewMapper = Mapper.Map<List<t_sp_attachment_DTO>>(dbResult);
                objResponseModel.Status = true;
                objResponseModel.Message = EnumHelper.GetDescription(Enums.ResponseType.Success);
                objResponseModel.TotalRecords = viewMapper.Count;
                objResponseModel.Result = viewMapper;
        ...
        [HttpPost]
        public HttpResponseMessage DeleteAttachment(SPInputs inputs)
        {
            var objResponseModel = new ResponseModel();
            ...
            {
                inputs.RepId = Decrypt(inputs.Auth, true);
                string modelpath = _mainBLL.GetFilePath(inputs.SpfId);
                if (!_mainBLL.DeleteSPAttachment(inputs.SpfId))
                {
                    objResponseModel.Status = false;
                    objResponseModel.Message = "Null Attachment";
                    return Request.CreateResponse(HttpStatusCode.NotFound, objResponseModel);
                }
                File.Delete(HttpContext.Current.Server.MapPath(modelpath.Replace("/mtid/dist/", "/bas/")));
        ...
        [HttpPost]
        public IHttpActionResult PrintView(SPInputs inputs)
        {
            var objResponseModel = new ResponseModel();
            try
            {
