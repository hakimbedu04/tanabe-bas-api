...
{
    public class VisistOnlyPivotController : BaseController
    {
        private readonly ILoginBLL _loginbll;

        public VisistOnlyPivotController(ILoginBLL loginbll)
        {
            _loginbll = loginbll;
        }
        ...
        public IHttpActionResult GetReport(BaseInput inputs)
        {
            ...
            objResponseModel.Result = GetHost() + "/Report/VisitOnlyPivot.aspx?p=" + model.rep_position.Replace(" ", string.Empty) + "&r=" + model.rep_id.Replace(" ", string.Empty);
