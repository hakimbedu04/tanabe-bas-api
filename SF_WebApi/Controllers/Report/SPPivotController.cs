...
{
    public class SPPivotController : BaseController
    {
        private readonly ILoginBLL _loginbll;

        public SPPivotController(ILoginBLL loginbll)
        {
            _loginbll = loginbll;
        }
        ...
        public IHttpActionResult GetReport(BaseInput inputs)
        {
            ...
            objResponseModel.Result = GetHost() + "/Report/SPPivot.aspx?p=" + model.rep_position.Replace(" ", string.Empty) + "&n=" + model.rep_id.Replace(" ", string.Empty) + "&s=" + DateTime.Now.ToString("yyyy-MM-dd") + "&e=" + DateTime.Now.ToString("yyyy-MM-dd");
