...
{
    public class DoctorPivotController : BaseController
    {
        private readonly ILoginBLL _loginbll;
        
        public DoctorPivotController(ILoginBLL loginbll)
        {
            _loginbll = loginbll;
        }
        ...
        public IHttpActionResult GetReport(BaseInput inputs)
        {
            ...
            objResponseModel.Result = GetHost() + "/Report/DoctorPivot.aspx?d="+model.rep_region.Replace(" ", string.Empty)+"&p="+model.rep_position.Replace(" ", string.Empty)+"&i="+model.rep_id.Replace(" ", string.Empty);
