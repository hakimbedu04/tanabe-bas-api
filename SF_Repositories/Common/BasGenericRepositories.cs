...
{
    public class BasGenericRepositories<TEntity> : IBasGenericRepositories<TEntity> where TEntity : class
    {
        internal bas_trialEntities Context;
        ...
        public BasGenericRepositories(bas_trialEntities contextEntities)
        {
        ...
        public IEnumerable<TEntity> Get(int pageIndex, int pageSize, Expression<Func<TEntity, bool>> filter = null, Func<IQueryable<TEntity>, IOrderedQueryable<TEntity>> orderBy = null, string includeProperties = "")
        {
            ...
            query = includeProperties.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries).Aggregate(query, (current, includeProperty) => current.Include(includeProperty));
            var res = orderBy != null ? orderBy(query).AsNoTracking().Skip((pageIndex - 1) * pageSize).Take(pageSize).ToList() : query.AsNoTracking().Skip((pageIndex - 1) * pageSize).Take(pageSize).ToList();
