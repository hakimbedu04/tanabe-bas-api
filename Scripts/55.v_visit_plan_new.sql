...
/****** Object:  View [dbo].[v_visit_plan_new]    Script Date: 10/17/2019 10:37:50 AM ******/
SET ANSI_NULLS ON
GO
...
-- desc : optimize query, delete isshowntopic flag
	SELECT
		ISNULL(tgm.latitude,'0') as latitude, ISNULL(tgm.longitude,'0') as longitude,
		...
		0 as isShownTopic, 
		(
			CASE WHEN (dbo.t_visit.visit_date_plan between (SELECT DATEADD(wk, DATEDIFF(wk, 7, CURRENT_TIMESTAMP), 7)) AND (SELECT DATEADD(wk, DATEDIFF(wk, 6, CURRENT_TIMESTAMP), 6 + 7))) THEN 1 ELSE 0 END
		) as isShift,
